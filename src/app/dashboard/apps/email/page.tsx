                ...
                <ScrollArea className="flex-1">
                    <div className="divide-y">
                        {emails.map((email) => (
                            <div
                                key={email.id}
                                className={`p-4 flex gap-4 cursor-pointer hover:bg-muted/30 transition-colors ${!email.read ? 'bg-primary/5' : ''}`}
                            >
                                <div className="pt-1">
                                    <Star className={`h-4 w-4 ${email.starred ? 'text-amber-400 fill-amber-400' : 'text-muted-foreground/30'}`} />
                                </div>
